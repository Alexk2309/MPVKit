...
# Always run from project root so relative dist/... paths resolve
...
# Copy to thin directories (used by Sources/BuildScripts/create-combined-framework.sh)
echo ""
...
# Optionally create combined framework
if [ "$1" = "--combined" ] || [ "$1" = "-c" ]; then
    echo ""
    echo "🔗 Creating combined framework..."
    ./Sources/BuildScripts/create-combined-framework.sh
fi
...
echo "   • For release: Run './Sources/BuildScripts/create-combined-framework.sh'"
echo "   • Or use: './scripts/rebuild-libmpv.sh --combined' to do both"
